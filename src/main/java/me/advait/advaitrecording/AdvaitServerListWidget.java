...
public class AdvaitServerListWidget extends AlwaysSelectedEntryListWidget<AdvaitServerListWidget.Entry> {
    ...
    private final AdvaitServerScreen screen;
    private final List<AdvaitServerListWidget.ServerEntry> servers = Lists.newArrayList();
    private final AdvaitServerListWidget.Entry scanningEntry = new AdvaitServerListWidget.ScanningEntry();
    private final List<AdvaitServerListWidget.LanServerEntry> lanServers = Lists.newArrayList();

    public AdvaitServerListWidget(AdvaitServerScreen screen, MinecraftClient client, int width, int height, int top, int bottom, int entryHeight) {
    ...
    public void setSelected(@Nullable AdvaitServerListWidget.Entry entry) {
        super.setSelected(entry);
        this.screen.updateButtonActivationStates();
    }

    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        AdvaitServerListWidget.Entry entry = (AdvaitServerListWidget.Entry)this.getSelectedOrNull();
        return entry != null ? entry.keyPressed(keyCode, scanCode, modifiers) : super.keyPressed(keyCode, scanCode, modifiers);
    }

    protected void moveSelection(MoveDirection direction) {
        this.moveSelectionIf(direction, (entry) -> {
            return !(entry instanceof AdvaitServerListWidget.ScanningEntry);
        });
    }
    ...
    public void setServers(ServerList servers) {
        ...
        for(int i = 0; i < servers.size(); ++i) {
            this.servers.add(new AdvaitServerListWidget.ServerEntry(this.screen, servers.get(i)));
        }
    ...
    public void setLanServers(List<LanServerInfo> lanServers) {
        ...
        while(var2.hasNext()) {
            ...
            this.lanServers.add(new AdvaitServerListWidget.LanServerEntry(this.screen, lanServerInfo));
        }
    ...
    @Environment(EnvType.CLIENT)
    public static class ScanningEntry extends AdvaitServerListWidget.Entry {
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            int i = var10000 - 9 / 2;
            this.client.textRenderer.draw(matrices, AdvaitServerListWidget.LAN_SCANNING_TEXT, (float)(this.client.currentScreen.width / 2 - this.client.textRenderer.getWidth(AdvaitServerListWidget.LAN_SCANNING_TEXT) / 2), (float)i, 16777215);
            String string3;
    ...
    @Environment(EnvType.CLIENT)
    public abstract static class Entry extends net.minecraft.client.gui.widget.AlwaysSelectedEntryListWidget.Entry<AdvaitServerListWidget.Entry> {
        public Entry() {
        }
    }

    @Environment(EnvType.CLIENT)
    public class ServerEntry extends AdvaitServerListWidget.Entry {
        ...
        protected ServerEntry(AdvaitServerScreen screen, ServerInfo server) {
            this.screen = screen;
            this.server = server;
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            if (!this.server.online) {
                ...
                AdvaitServerListWidget.SERVER_PINGER_THREAD_POOL.submit(() -> {
                    try {
                        ...
                        this.server.ping = -1L;
                        this.server.label = AdvaitServerListWidget.CANNOT_RESOLVE_TEXT;
                    } catch (Exception var3) {
                        this.server.ping = -1L;
                        this.server.label = AdvaitServerListWidget.CANNOT_CONNECT_TEXT;
                    }
            ...
            if (bl) {
                s = 5;
                text5 = AdvaitServerListWidget.INCOMPATIBLE_TEXT;
                ...
                if (this.server.ping < 0L) {
                    text5 = AdvaitServerListWidget.NO_CONNECTION_TEXT;
                ...
                text5 = AdvaitServerListWidget.PINGING_TEXT;
                list5 = Collections.emptyList();
            }
            ...
            if (this.icon == null) {
                this.draw(matrices, x, y, AdvaitServerListWidget.UNKNOWN_SERVER_TEXTURE);
            } else {
            ...
            if (this.client.options.touchscreen || hovered) {
                RenderSystem.setShaderTexture(0, AdvaitServerListWidget.SERVER_SELECTION_TEXTURE);
        ...
        private boolean isNewIconValid(@Nullable String newIconUri) {
            if (newIconUri == null) {
                ...
                try {
                    ...
                    AdvaitServerListWidget.LOGGER.error("Invalid icon for server {} ({})", this.server.name, this.server.address, var3);
                    return false;
                }
            }
        ...
        public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
            if (Screen.hasShiftDown()) {
                AdvaitServerListWidget multiplayerServerListWidget = this.screen.serverListWidget;
        ...
        private void swapEntries(int i, int j) {
            ...
            AdvaitServerListWidget.Entry entry = (AdvaitServerListWidget.Entry)this.screen.serverListWidget.children().get(j);
            this.screen.serverListWidget.setSelected(entry);
            AdvaitServerListWidget.this.ensureVisible(entry);
        }

        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            double d = mouseX - (double) AdvaitServerListWidget.this.getRowLeft();
            double e = mouseY - (double) AdvaitServerListWidget.this.getRowTop(AdvaitServerListWidget.this.children().indexOf(this));
            if (d <= 32.0D) {
        ...
        public Text getNarration() {
            return new TranslatableText("narrator.select", new Object[]{this.server.name});
        }
    }

    @Environment(EnvType.CLIENT)
    public static class LanServerEntry extends net.minecraft.client.gui.screen.multiplayer.MultiplayerServerListWidget.Entry {
